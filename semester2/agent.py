...
class StockNetwork(nn.Module):
    ...
    def sample_action(self, state, goal, days):
        ...
        pdf_total_stock_target=(1 / (np.sqrt(2 * np.pi) * std)) * torch.exp(-0.5 * ((total_stock_target - mean) / std) ** 2)
        ...
        log_density=torch.log(pdf_total_stock_target) #en réalité c'est la log_likelihood mais en 1D c'est la meme chose donc on la nomme comme ca pour enlever toute ambiguité lorque l'on va utiliser la policy gradient method 
