import numpy as np
import torch
from agent_copy import StockNetwork
...
def simulate_episode(model, S0, V0, mu, kappa, theta, sigma, rho, days, goal, flag, batch_size=2):
    ...
    for t in range(days):
        ...
        with torch.no_grad() if not flag else torch.enable_grad():
            if flag:  # Mode entraînement avec sampling
                output = model.forward(state_tensor)
                mean, bell_param = output[:, 0], output[:, 1]
                std = (goal / days) * 0.1
                
                # Sampling gaussien pour l'achat d'actions
                total_stock_target = mean + std * torch.randn_like(mean)
                
                # Sampling bernoulli pour l'arrêt prématuré
                u = torch.rand_like(bell_param)
                bell = (u < bell_param).float()

                # Calcul des log probabilités
                log_p_gaussian = -0.5 * ((total_stock_target - mean)/std)**2 - torch.log(std * np.sqrt(2 * np.pi))
                log_p_bell = bell * torch.log(bell_param + 1e-8) + (1 - bell) * torch.log(1 - bell_param + 1e-8)
                log_density = log_p_gaussian + log_p_bell

            else:  # Mode évaluation déterministe
                if isinstance(model, Net):
                    ...
                    output = model.forward(state_tensor)
                    total_stock_target = output[:, 0]
                    bell = (output[:, 1] > 0.5).float()
        # MAJ des états
        q_n[t+1,:] = total_stock_target.squeeze() if t < days-1 else goal # * (goal - q_n[t, :]) if t < days - 1 else goal
        v_n = q_n[t+1, :] - q_n[t, :]
        total_spent[t+1, :] = total_spent[t, :] + v_n * S_n[t+1, :]
        log_densities[t, :] = log_density.squeeze() if log_density is not None else 0
        #print(prob)
        #probabilities[t, :] = prob #np.exp(-prob)
        actions[t, :] = v_n
        bell_signals[t, :] = bell.squeeze()
...
def train_model(model, simulate_episode, num_episodes, S0,V0, mu,kappa, theta, sigma,rho, days, goal, batch_size=2):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if episode % 50 == 0:
            print(f"Episode {episode}: Average Episode Payoff {episode_payoff.mean().item()}, Loss {loss.item()}")
...
"""
model_name = input("Quel modèle voulez-vous utiliser ? (par exemple : Net(1), StockNetwork(0), etc.) : ").strip()
