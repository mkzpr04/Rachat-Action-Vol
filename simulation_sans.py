...
def simulate_episode(model, S0, V0, mu, kappa, theta, sigma, rho, days, goal, flag, batch_size=2):
    ...
    for t in range(days):
        if isinstance(model, Net) or isinstance(model, StockNet):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                print(f"Gradient de {name} : {param.grad.norm().item()}")

        optimizer.step()

def evaluate_single_episode(model, S0, V0, mu, kappa, theta, sigma, rho, N, Q, batch_size=2):
    results = simulate_episode(model, S0, V0, mu, kappa, theta, sigma, rho, days, goal, flag=False, batch_size=2)
...
model_name = input("Quel modèle voulez-vous utiliser ? (par exemple : Net(0), StockNetwork(1), StockNet(2)) : ").strip()
