"""
rendre le batch générique 
enlever les paramtres du modele d'heston car sert a rien 
question sur pourquoi lorsque le payoff full 'nan' ca marche sinon si c'est torch zero
payoff moyen de 0
...
def simulate_episode(model, S0, sigma, days, goal, flag, batch_size=Batch):
    #total spent =X_n
    #v_n= actions
...
def plot_episodes(S_n, A_n, q_n_values, cloche_n_values, sigma_values):
    plt.figure(figsize=(14, 7))
    fig, ax1 = plt.subplots(figsize=(14, 7))
    ...
    ax1.plot(S_n, label="S_n (Prix de l'action au jour n)", color="blue")
    ax1.plot(A_n, label="A_n (Prix moyen des actions aux jours n)", color="green", linestyle='--')
    ...
    for i, (q_n, cloche_n, sigma) in enumerate(zip(q_n_values, cloche_n_values, sigma_values)):
        ax2.plot(q_n, label=f"q_n (Sigma={sigma})", color=colors[i], linestyle=':')
        for j, cloche_value in enumerate(cloche_n):
            if float(cloche_value.item()) >= 1:
                ax1.axvline(x=j, color=colors[i], linestyle='--', label=f"cloche_n = 1 (Sigma={sigma})" if j == 0 else "")
                break
    ...
    plt.title("Évolution de S_n, A_n, q_n et cloche_n au fil du temps pour différentes valeurs de sigma")
    plt.grid(True)
    plt.show()


sigma_values = [0.1, 0.3, 0.6, 2.0, 5.0]
S_n_values = []
A_n_values = []
q_n_values = []
cloche_n_values = []
...
# Simuler un épisode pour obtenir S_n et A_n
S_n, A_n, q_n, total_spent, actions, log_densities, cloche_n, episode_payoff = simulate_episode(
    model, S0, sigma_values[0], days, Q, flag=False, batch_size=Batch
)
S_n_values.append(S_n[:, 0])  # Stocker les résultats pour le premier batch
A_n_values.append(A_n[:, 0])

for sigma in sigma_values:
    # Fixer la graine aléatoire pour chaque simulation
    np.random.seed(seed)
    torch.manual_seed(seed)
    ...
    q_n_values.append(q_n[:, 0])
    cloche_n_values.append(cloche_n[:, 0])

    # Exportation des résultats du premier batch
    export_csv(actions[:, 0], episode_payoff[0], S_n[:,0], A_n[:, 0], q_n[:, 0], total_spent[:, 0], f"episode_sans_heston_sigma_{sigma}.csv")

# Utiliser les valeurs de S_n et A_n du premier sigma pour le tracé
plot_episodes(S_n_values[0], A_n_values[0], q_n_values, cloche_n_values, sigma_values)